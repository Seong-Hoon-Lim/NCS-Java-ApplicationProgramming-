...
public class CheckingAccount extends Account {
	...
	public CheckingAccount(String generateAccountNum) {
		super(generateAccountNum);
	}
	
	public CheckingAccount(String generateAccountNum, double balance, double overdraftProtection) {
		super(balance);
	}
	...
	public void withdraw (double amount) throws OverdraftException {
		...
		if (balance <= 0) {
			if (overdraftProtection + balance < 0) {
				throw new OverdraftException("마이너스 계좌 한도를 초과 하였습니다.");
			}
			else {
				balance -= amount;
